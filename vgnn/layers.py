...
class TensorFusionLayer(nn.Module):
    ...
    def forward(self, inputdata):
        ...
        inputdata = F.dropout(inputdata, self.dropout, training=self.training)#[96,5]
        # print('111',inputdata.shape)
        inputdata_l = inputdata.unsqueeze(2)
        inputdata_r = inputdata.unsqueeze(1)
        # print("inputdata_l shape:", inputdata_l.shape)
        # print("inputdata_r shape:", inputdata_r.shape)
        # # inputdata_l = inputdata
        # inputdata_r = inputdata
        # inputdata_l = inputdata_l.permute(0, 2, 1)  # 将形状改为 [96, 1, 5]
