...
class Graph_Attention(nn.Module):
    ...
    def __init__(self, in_features, out_features, dropout, alpha ,concat=True, residual=False):
        ...
        self.W_static = nn.Parameter(torch.zeros(96,96).type(torch.FloatTensor), requires_grad=True)
    ...
    def get_relation(self, input_r, relation_static = None):
        ...
        if not isinstance(self.coef_revise,torch.Tensor):
            self.coef_revise = torch.zeros(96, 96, device = input_r.device) + 1.0 - torch.eye(96, 96,device = input_r.device)
        # print("coefs shape:", coefs.shape)
        # print("self.coef_revise shape:", self.coef_revise.shape)
    ...
    def forward(self, input_s, input_r, relation_static = None):
        ...
        # 计算门控
        # gate = self.get_gate(seq_s)
        # gate = F.dropout(gate, self.dropout, training=self.training)
        # # 融合
        # coefs = F.softmax(coefs_eye, dim=1)
        # coefs = F.dropout(coefs, self.dropout, training=self.training)
        # print('1111',coefs.shape,seq_fts_s.shape)
        #
        # output = torch.matmul(coefs, seq_fts_s)
        # output = output * gate
        #
        # if self.residual:
        #     output += seq_s.squeeze(0)
        #
        # return output
